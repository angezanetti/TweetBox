...
// Module used to tweet the status


	fs.readFile('json/idTwitter.json', function (err, data) {
  		if (err) throw err;
  		idApi = JSON.parse(data);
      // Get all the Oauth data from the JSON file
		...
		var time = new Date();										
    // We get the date to make the tweet unique
		...
		var oauthConsumer = new OAuth(
          ...
          // use the tweasy lib to send it to the twitter API
   	 			access_token : my_access_token,
  				access_token_secret : my_access_token_secret,
			});
			twitterClient.updateStatus("Il est " + time + ", " + status,
          // We add the date to the status
		  	function(er, resp){
				if (!er) {
